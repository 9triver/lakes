...
class ImageryInventoryMixin:
    def _load_tci_index(self) -> dict[str, dict]:
        return load_tci_index(self.region)

    def _load_user_tci_rows(self) -> dict[str, list[dict]]:
        return load_user_tci_rows(self.region)

    def _load_active_imagery(self) -> dict[str, str]:
        return load_active_imagery(self.region.active_imagery)

    def _save_active_imagery(self) -> None:
        save_active_imagery(self.region.active_imagery, self.active_imagery)

    def _rebuild_effective_tci(self) -> None:
        effective = {}
        tiles = (
            set(self.base_tci_by_tile)
            | set(self.user_tci_rows)
        ...
        for tile in tiles:
            ...
            if (
                base
                and base.get("product") == active_product
    ...
    def _active_imagery_key(self, tile: str, site=None) -> str:
        ...
        if site_id and any(
            row.get("site_id") == site_id for row in self.user_tci_rows.get(tile, [])
        ):
    ...
    def _active_imagery_row(self, tile: str, site=None) -> dict | None:
        ...
        return next(
            (
                ...
                and (
                    not site_id
                    or not row.get("site_id")
                    or row.get("site_id") == site_id
                )
                and row.get("tci_path")
                and row["tci_path"].exists()
            ),
    ...
    def _imagery_asset_meta(self, row: dict, site_id: str = "") -> dict:
        ...
        if is_local_imagery_source(source):
            ...
            asset_type, asset_scope, asset_label = (
                "preloaded_tile",
                "tile",
                "预置 Sentinel tile",
            )
    ...
    def _imagery_product_payload(
        ...
        source = clean_optional(row.get("source")) or (
            "preloaded" if preloaded else "user_download"
        )
    ...
    def _local_imagery_rows_for_site(self, site) -> list[dict]:
        ...
        rows = [
            ...
            if is_local_imagery_source(row.get("source"))
    ...
    def _active_local_imagery_row(self, site) -> dict | None:
        rows = [
            row
            for row in self._local_imagery_rows_for_site(site)
            if row.get("tci_path") and row["tci_path"].exists()
        ]
        if not rows:
            return None
        ...
        selected = next(
            (
                ...
                for row in rows
                if row.get("product") == active_product
                or row.get("product_id") == active_product
            ),
            None,
        )
        if selected is not None:
        ...
        selected = next(
            (
                row
                for row in rows
                if self.active_imagery.get(f"{_site_id(site)}:{row.get('tile', '')}")
                == row.get("product")
            ),
            None,
        )
    ...
    def imagery_inventory_summary(self) -> dict:
        ...
        return {
            ...
            "active_tile_count": len(
                {key.rsplit(":", 1)[-1] for key in self.active_imagery}
            ),
            "product_count": sum(len(rows) for rows in self.user_tci_rows.values())
            + len(self.base_tci_by_tile),
        }
    ...
    def imagery_products_for_tile(self, tile: str, site=None) -> list[dict]:
        ...
        if base and not has_site_products:
            products.append(
                self._imagery_product_payload(
                    base, tile, active_key, site_id=site_id, preloaded=True
                )
            )
        for row in self.user_tci_rows.get(tile, []):
            if site_id and row.get("site_id") and row.get("site_id") != site_id:
                continue
            products.append(
                self._imagery_product_payload(row, tile, active_key, site_id=site_id)
            )
        return products

    def local_product_status(
        ...
        for rows in self.user_tci_rows.values():
            for row in rows:
                if (
                    product_id and clean_optional(row.get("product_id")) == product_id
                ) or (
    ...
    def set_active_imagery(self, tile: str, product_name: str, site=None) -> dict:
        ...
        with self._lock:
            ...
            selected = next(
                (row for row in candidates if row.get("product") == product_name), None
            )
            if selected is None:
    ...
    def set_active_site_imagery(
        self, site, asset_id: str = "", product_name: str = ""
    ) -> dict:
        ...
        selected = next(
            (
                ...
                if (
                    asset_id
                    and (
                        row.get("product_id") == asset_id
                        or row.get("product") == asset_id
                    )
                )
        ...
        if selected is None:
            raise KeyError(
                f"Imagery asset not found for site {_site_id(site)}: {asset_id or product_name}"
            )
        with self._lock:
            ...
            self._rebuild_effective_tci()
            self.tci_footprints = self._load_tci_footprints()
        payload = self._imagery_product_payload(
            selected,
            selected.get("tile", ""),
            f"site:{_site_id(site)}",
            site_id=_site_id(site),
        )
        payload["active"] = True
        return {"site_id": _site_id(site), "asset": payload}

    def register_downloaded_product(
        self, product: dict, safe_dir: Path, tci_path: Path
    ) -> dict:
        tile = product_tile_name(product.get("name", "")) or product_tile_name(
            str(safe_dir.name)
        )
