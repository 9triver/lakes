...
import { useContextWater, useEsaLayer, useJrcLayer, useLake, useLakes, useLocalLabel, useLocalLabels, useTileMeta } from "../features/lakes/api";
import { LakeFilterControls } from "../features/lakes/LakeFilterControls";
import { useRegions } from "../features/regions/api";
import { LakeMap, type LakeMapHandle } from "../features/map/LakeMap";
...
import type { LakeFilters, TrainingPatch, TrainingSample } from "../api/types";
...
export function App() {
  ...
  const [filters, setFilters] = useState<LakeFilters>(() => ({
    water_type: initialSearch.get("water_type") || "",
    area_bucket: initialSearch.get("area_bucket") || "",
    has_name: initialSearch.get("has_name") || "",
    has_tci: initialSearch.get("has_tci") || "",
    polygon_quality: initialSearch.get("polygon_quality") || "",
    metadata_quality: initialSearch.get("metadata_quality") || "",
  }));
  ...
  const mapRef = useRef<LakeMapHandle>(null);
  const region = useWorkbenchStore((state) => state.region);
  const setRegion = useWorkbenchStore((state) => state.setRegion);
  const lakeRoute = location.pathname.match(/^\/regions\/([^/]+)\/lakes\/([^/]+)$/);
  const lakeListRoute = location.pathname.match(/^\/regions\/([^/]+)\/lakes$/);
  ...
  const modelLakeRegion = routeParams.get("lake_region") || (modelRoute?.[1] === "all" ? "" : modelRoute?.[1] || "");
  const isLakeWorkspace = !isTraining && !isModelValidation;
  const regions = useRegions();
  const lakes = useLakes(region, query, filters);
  const lakeItems = lakes.data?.pages.flatMap((page) => page.items) || [];
  const lakeTotal = lakes.data?.pages[0]?.total;
  const lake = useLake(selectedRegion, selectedLakeId);
  ...
  useEffect(() => {
    if (region && location.pathname === "/") navigate(`/regions/${region}/lakes${lakeSearch}`, { replace: true });
  ...
  const navigateToLake = useCallback((item: TrainingSample | TrainingPatch) => {
    navigate(`/regions/${item.region || region}/lakes/${item.lake_id}${lakeSearch}`);
  }, [lakeSearch, navigate, region]);

  const handleRegionChange = (nextRegion: string) => {
    setRegion(nextRegion);
    navigate(isTraining ? `/regions/${nextRegion}/training/${trainingView || "samples"}` : isModelValidation ? `/regions/${nextRegion}/model` : `/regions/${nextRegion}/lakes${lakeSearch}`);
  };
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "360px minmax(0, 1fr)" }, height: "100vh", bgcolor: "background.default" }}>
      <Box component="aside" sx={{ display: { xs: selectedLakeId || isTraining || isModelValidation ? "none" : "block", md: "block" }, borderRight: 1, borderColor: "divider", bgcolor: "background.paper", overflow: "auto" }}>
        <Box sx={{ p: 2, display: "grid", gap: 1.5, position: "sticky", top: 0, bgcolor: "background.paper", zIndex: 1 }}>
          ...
          <FormControl fullWidth>
            ...
            <Select labelId="region-label" label="区域" value={region} onChange={(event) => handleRegionChange(event.target.value)}>
              ...
              {(regions.data?.items || []).map((item) => <MenuItem key={item.key} value={item.key}>{item.name} ({item.lake_count})</MenuItem>)}
            </Select>
          </FormControl>
          <List disablePadding sx={{ mx: -1 }}>
            <ListItemButton selected={isLakeWorkspace} onClick={() => navigate(`/regions/${region}/lakes${lakeSearch}`)}><ListItemIcon sx={{ minWidth: 34 }}><Images size={18} /></ListItemIcon><ListItemText primary="湖泊" /></ListItemButton>
          ...
          {isLakeWorkspace && <>
            <TextField fullWidth placeholder="名称 / OSM ID" value={query} onChange={(event) => setQuery(event.target.value)} slotProps={{ input: { startAdornment: <Search size={17} /> } }} />
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1 }}><LakeFilterControls value={filters} onChange={setFilters} /></Box>
            <Typography variant="caption" color="text.secondary">{lakeTotal != null ? `${lakeTotal} 个水体，显示 ${lakeItems.length} 个` : "加载中"}</Typography>
          </>}
        </Box>
        {isLakeWorkspace && (lakes.isLoading ? <Box sx={{ p: 3, textAlign: "center" }}><CircularProgress size={24} /></Box> : lakes.isError ? <Typography color="error" sx={{ p: 2 }}>{lakes.error.message}</Typography> : (
          <List disablePadding>
            {!lakeItems.length && <Box sx={{ p: 3, textAlign: "center" }}><Typography variant="body2">没有符合条件的水体</Typography></Box>}
            {lakeItems.map((lake, index) => (
              <ListItemButton key={`${lake.region || region}:${lake.object_id}:${index}`} divider selected={lake.object_id === selectedLakeId} onClick={() => navigate(`/regions/${lake.region || region}/lakes/${lake.object_id}${lakeSearch}`)}>
                <ListItemText primary={<Box sx={{ display: "flex", alignItems: "center", gap: .75 }}><Typography variant="body2" noWrap sx={{ flex: 1 }}>{lake.display_name || lake.name || lake.object_id}</Typography>{region === "all" && <Chip size="small" label={lake.region_name || lake.region} />}</Box>} secondary={`${lake.area_km2.toFixed(2)} km² · ${(lake.tiles || []).join(", ")}${lake.has_tci ? " · TCI" : ""}`} />
              </ListItemButton>
            ))}
      ...
      <Box component="main" sx={{ minWidth: 0, display: { xs: selectedLakeId || isTraining || isModelValidation ? "grid" : "none", md: "grid" }, gridTemplateRows: selectedLakeId ? "auto minmax(0, 1fr) auto" : isTraining ? "auto minmax(0, 1fr)" : "1fr", color: "text.secondary" }}>
        {isModelValidation ? <ModelValidationView scope={region} routeLakeId={modelLakeId} routeLakeRegion={modelLakeRegion} routeModel={routeModel} onBack={() => navigate(`/regions/${region}/lakes${lakeSearch}`)} onRouteModel={(model) => navigate(`/regions/${region}/model?model=${encodeURIComponent(model)}`, { replace: true })} onRouteResult={(lakeId, lakeRegion, model) => navigate(`/regions/${region}/model/${encodeURIComponent(lakeId)}?model=${encodeURIComponent(model)}${region === "all" ? `&lake_region=${encodeURIComponent(lakeRegion)}` : ""}`)} /> : isTraining ? <>
          <Box sx={{ borderBottom: 1, borderColor: "divider", bgcolor: "background.paper", display: "flex", alignItems: "center", px: { xs: 1, sm: 2 } }}>
            <IconButton sx={{ display: { md: "none" }, mr: .5 }} onClick={() => navigate(`/regions/${region}/lakes`)}><ArrowLeft size={19} /></IconButton>
            ...
            <Button sx={{ ml: "auto", display: { xs: "none", sm: "inline-flex" } }} startIcon={<Images size={16} />} onClick={() => navigate(`/regions/${region}/lakes`)}>浏览湖泊</Button>
          </Box>
        ...
        </> : !selectedLakeId ? <Box sx={{ display: "grid", placeItems: "center" }}><Typography>选择一个水体</Typography></Box> : lake.isLoading ? <Box sx={{ display: "grid", placeItems: "center" }}><CircularProgress size={28} /></Box> : lake.data ? <>
          <Box sx={{ px: 2, py: 1.25, borderBottom: 1, borderColor: "divider", bgcolor: "background.paper", display: "flex", alignItems: "center", gap: 1 }}>
            <IconButton sx={{ display: { md: "none" } }} onClick={() => navigate(`/regions/${region}/lakes${lakeSearch}`)}><ArrowLeft size={19} /></IconButton>
            <Box><Typography variant="subtitle1" color="text.primary">{lake.data.display_name || lake.data.name || lake.data.object_id}</Typography><Typography variant="caption">{lake.data.area_km2.toFixed(2)} km² · {lake.data.water_type || "water"}</Typography></Box>
          </Box>
          <LakeMap
            ref={mapRef}
            lake={lake.data}
        ...
        </> : <Box sx={{ display: "grid", placeItems: "center" }}><Typography color="error">湖泊加载失败</Typography></Box>}
      </Box>
    </Box>
  );
}
