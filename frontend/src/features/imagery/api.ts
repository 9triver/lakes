...
export function useImagery(region: string, lakeId: string) {
  return useQuery({ queryKey: ["imagery", region, lakeId], queryFn: () => getJson<ImageryResponse>(`/api/regions/${encodeURIComponent(region)}/lakes/${encodeURIComponent(lakeId)}/imagery`), enabled: Boolean(region && lakeId) });
}

export function useSentinelTiles(region: string, lakeId: string) {
  return useQuery({ queryKey: ["sentinel-tiles", region, lakeId], queryFn: async () => (await getJson<{ tiles: SentinelTile[] }>(`/api/regions/${encodeURIComponent(region)}/lakes/${encodeURIComponent(lakeId)}/sentinel/tiles`)).tiles, enabled: Boolean(region && lakeId) });
}
...
export async function searchSentinelProducts(region: string, lakeId: string, params: { tile: string; start: string; end: string; cloud: number }) {
  const query = new URLSearchParams({ tile: params.tile, lake_id: lakeId, start: params.start, end: params.end, cloud: String(params.cloud), product_type: "MSIL1C", limit: "50" });
...
export function useSetActiveImagery(region: string, lakeId: string) {
  const client = useQueryClient();
  return useMutation({
    mutationFn: ({ tile, product }: { tile: string; product: string }) => postJson(`/api/regions/${encodeURIComponent(region)}/lakes/${encodeURIComponent(lakeId)}/imagery/active`, { tile, product }),
    onSuccess: async () => {
      await Promise.all([
        client.invalidateQueries({ queryKey: ["imagery", region, lakeId] }),
        client.invalidateQueries({ queryKey: ["tile-meta", region, lakeId] }),
