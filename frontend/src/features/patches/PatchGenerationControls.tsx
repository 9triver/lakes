...
export function PatchGenerationControls({ workspaceId, scope }: { workspaceId: string; scope: string }) {
  const [jobId, setJobId] = useState("");
  const start = useStartPatchExport(workspaceId, scope);
  const job = usePatchExportJob(workspaceId, scope, jobId);
  ...
  return <Box sx={{ px: 2, py: 1.25, borderBottom: 1, borderColor: "divider", bgcolor: "background.paper" }}>
    <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
      <Box sx={{ mr: "auto" }}><Typography variant="subtitle2">逻辑 Patch</Typography><Typography variant="caption" color="text.secondary">当前工作区独立的 512 x 512 底稿，重建会保留审核状态</Typography></Box>
      <Button variant="contained" startIcon={<Grid2X2Plus size={16} />} disabled={running} onClick={submit}>重建当前工作区 Patch</Button>
    </Box>
