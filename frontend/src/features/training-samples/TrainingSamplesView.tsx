...
import { Box, Button, Chip, CircularProgress, Table, TableBody, TableCell, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
function SampleRow({ sample, scope, onLocate }: { sample: TrainingSample; scope: string; onLocate: (sample: TrainingSample) => void }) {
  ...
  useEffect(() => { setNotes(sample.notes || ""); }, [sample.notes]);
  return <TableRow hover>
    ...
    <TableCell><Box sx={{ display: "flex", gap: .5 }}><Button startIcon={<LocateFixed size={15} />} onClick={() => onLocate(sample)}>定位</Button><Button startIcon={<Save size={15} />} disabled={update.isPending} onClick={() => update.mutate({ sample, changes: { notes } })}>保存</Button><Button color="error" startIcon={<Trash2 size={15} />} disabled={remove.isPending} onClick={() => { if (confirm("删除这个训练样本记录？")) remove.mutate(sample); }}>删除</Button></Box></TableCell>
  </TableRow>;
}
