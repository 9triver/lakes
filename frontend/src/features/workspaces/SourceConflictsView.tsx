...
function ConflictRow({ workspaceId, item }: { workspaceId: string; item: NonNullable<ReturnType<typeof useSourceConflicts>["data"]>["items"][number] }) {
  const [selected, setSelected] = useState<string[]>(item.candidate_variant_ids);
  const resolve = useResolveSourceConflict(workspaceId);
...
export function SourceConflictsView({ workspaceId }: { workspaceId: string }) {
  const query = useSourceConflicts(workspaceId);
  ...
  return <Box sx={{ height: "100%", overflow: "auto" }}><Alert severity="warning" sx={{ borderRadius: 0 }}>{query.data.total} 个 site 需要确认来源</Alert>{query.data.items.map((item) => <ConflictRow key={item.site_id} workspaceId={workspaceId} item={item} />)}</Box>;
}
