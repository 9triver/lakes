...
import { useContextWater, useEsaLayer, useJrcLayer, useLake, useLocalLabel, useLocalLabels, useTileMeta } from "../lakes/api";
import { ImageryPanel } from "../imagery/ImageryPanel";
import { useSentinelTiles } from "../imagery/api";
import { LakeMap, type LakeMapHandle } from "../map/LakeMap";
...
function ValidationWorkspace({ result, threshold }: { result: NonNullable<ReturnType<typeof useRandomModelValidation>["data"]>; threshold: number }) {
  const region = result.lake?.region || result.region;
  const lakeId = result.lake_id;
  ...
  const lake = useLake(region, lakeId);
  ...
  if (!lake.data) return <Alert severity="error">无法加载推理水体</Alert>;
  return <Box sx={{ display: "grid", gridTemplateRows: "minmax(420px,1fr) auto", minHeight: 0 }}>
    <LakeMap ref={mapRef} lake={lake.data} tileMeta={tileMeta.data} sentinelTiles={sentinelTiles.data} contextOsm={context.data?.sources.osm} contextHydro={context.data?.sources.hydrolakes} esa={esa.data} jrc={jrc.data} localLabel={localLabel.data} localLabels={localLabels.data || []} selectedLocalLabel={selectedLocalLabel} onLocalLabelChange={setSelectedLocalLabel} jrcThreshold={jrcThreshold} onJrcThresholdChange={setJrcThreshold} modelPrediction={result.prediction} />
...
export function ModelValidationView({ scope, routeLakeId = "", routeLakeRegion = "", routeModel = "", onBack, onRouteResult, onRouteModel }: ModelValidationViewProps) {
  ...
  const randomModelMatches = Boolean(random.data && (random.data.model.key === selectedModel || selectedModel === `${random.data.lake?.region || random.data.region}/${random.data.model.key}`));
  const randomMatchesRoute = Boolean(random.data && random.data.lake_id === routeLakeId && (random.data.lake?.region || random.data.region) === predictionRegion && randomModelMatches);
