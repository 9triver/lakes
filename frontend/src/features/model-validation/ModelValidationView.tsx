...
function ValidationWorkspace({ profileId, result, threshold }: { profileId: string; result: NonNullable<ReturnType<typeof useRandomModelValidation>["data"]>; threshold: number }) {
  ...
  const site = useSite(region, siteId, profileId);
...
export function ModelValidationView({ profileId, scope, routeSiteId = "", routeSiteRegion = "", routeModel = "", onBack, onRouteResult, onRouteModel }: ModelValidationViewProps) {
  const [visibility, setVisibility] = useState<"current" | "all">("current");
  const models = useValidationModels(profileId, scope, visibility);
  const random = useRandomModelValidation(profileId, scope);
  ...
  const prediction = useSiteModelPrediction(profileId, predictionRegion, routeSiteId, selectedModel, threshold, Boolean(routeSiteId && !randomMatchesRoute));
  ...
  return <Box sx={{ height: "100%", minHeight: 0, overflow: "auto", display: "grid", gridTemplateRows: "auto minmax(0,1fr)" }}>
    <Box sx={{ p: { xs: 1.5, sm: 2 }, bgcolor: "background.paper", borderBottom: 1, borderColor: "divider" }}>
      <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <ToggleButtonGroup exclusive size="small" value={visibility} onChange={(_, value) => { if (value) { setVisibility(value); setSelectedModel(""); } }}><ToggleButton value="current">当前用户</ToggleButton><ToggleButton value="all">全部用户</ToggleButton></ToggleButtonGroup>
