...
export function ModelWorkbench({ workspaceId, scope, stage, defaults = {}, onStageChange, onTrainingDataGenerated, ...validationProps }: ModelWorkbenchProps) {
  return <Box sx={{ height: "100%", minHeight: 0, display: "grid", gridTemplateRows: "auto minmax(0,1fr)", overflow: "hidden" }}>
    <Box sx={{ px: { xs: 1.5, sm: 2 }, bgcolor: "background.paper", borderBottom: 1, borderColor: "divider", display: "flex", alignItems: "center", gap: 2 }}>
      <Typography variant="h6" color="text.primary" sx={{ py: 1.25 }}>模型</Typography>
    ...
    <Box sx={{ minHeight: 0, overflow: "hidden" }}>
      {stage === "train" ? <TrainingView workspaceId={workspaceId} scope={scope} defaults={defaults} /> : <ModelValidationView workspaceId={workspaceId} scope={scope} onTrainingDataGenerated={onTrainingDataGenerated} {...validationProps} />}
    </Box>
  </Box>;
}
